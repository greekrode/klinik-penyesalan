...
HTML import publishes a fully designed page as-is. Choose **IMPORT HTML** in the editor and pick a self-contained `.html` file (1.5 MB or smaller — the document is embedded in the server-rendered article response, which must stay within Vercel's payload limits, so host large images in the media bucket or externally rather than as data URIs) with its CSS and SVG graphics embedded in the file. The document is sanitized (scripts, forms, and event handlers are removed), stored as the post body with a `<!doctype html>` marker, and rendered on the public article page inside a sandboxed frame that sizes itself to the content, so the document's styles never conflict with the site's. The title, excerpt, and thumbnail fields still control post cards, link previews, and the article header; if the title or excerpt fields are empty, they are pre-filled from the document's `<title>` and meta description. Use **SWITCH TO EDITOR** to convert the post back to rich text.
